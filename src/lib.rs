...
// Constants
const PHPMD_CONFIG_FILES: &[&str] = &[".phpmd.xml", "phpmd.xml", ".phpmd.xml.dist", "phpmd.xml.dist", "ruleset.xml"];
...
impl PhpmdLspServer {
    ...
    fn download_binary(&self, binary_name: &str) -> Result<String> {
        // Use the same pattern as PHPCS extension
...
impl zed::Extension for PhpmdLspExtension {
    ...
    fn language_server_initialization_options(
        ...
        // Determine ruleset to use (priority order: config file -> settings -> env -> default)
        let mut ruleset_to_use: Option<String> = None;
        
        // Try to find phpmd configuration file first (highest priority)
        if let Some(config_file) = Self::find_phpmd_config(worktree) {
            // For PHPMD, when we have a config file, we pass its path
            ruleset_to_use = Some(config_file);
        }
        
        // Check for user-configured ruleset from settings.json
        if ruleset_to_use.is_none() {
            if let Some(settings) = user_settings.as_ref() {
                // Support both string and array formats for rulesets
                if let Some(ruleset_value) = settings.get("ruleset") {
                    match ruleset_value {
                        // Single ruleset as string
                        zed::serde_json::Value::String(ruleset) => {
                            if !ruleset.trim().is_empty() {
                                ruleset_to_use = Some(ruleset.clone());
                            }
                        },
                        ...
                        zed::serde_json::Value::Array(rulesets) => {
                            ...
                            if !ruleset_strings.is_empty() {
                                ...
                                ruleset_to_use = Some(combined_rulesets);
                            }
                        },
                        _ => {}
                    }
        ...
        // Fall back to environment variable for ruleset
        if ruleset_to_use.is_none() {
            if let Ok(env_ruleset) = env::var("PHPMD_RULESET") {
                if !env_ruleset.trim().is_empty() {
                    ruleset_to_use = Some(env_ruleset);
                }
            }
        }
        
        // Default to common rulesets if nothing is configured
        if ruleset_to_use.is_none() {
            ruleset_to_use = Some("cleancode,codesize,controversial,design,naming,unusedcode".to_string());
        }
        
        // Pass the ruleset to the LSP server
        if let Some(ruleset) = ruleset_to_use {
            options.insert("ruleset".to_string(), zed::serde_json::Value::String(ruleset.clone()));
        }
        
        // Pass output format preference (json for easier parsing)
        options.insert("format".to_string(), zed::serde_json::Value::String("json".to_string()));
...
impl PhpmdLspExtension {
    ...
    fn download_phar_if_needed(phar_name: &str) -> Result<String> {
        // Use the same pattern as PHPCS extension for consistency
        let version_dir = format!("phpmd-{}", VERSION);
        ...
        // After extraction, the file should be in the version directory
        if !fs::metadata(&phar_path).is_ok() {
